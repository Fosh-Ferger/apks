...
bool GetFWTime(WCHAR *path, FILETIME *ret)
{
	...
	{
		std::cout << "Ошибка! Нет доступа к файлу!\n";
		return 0;
	}
...
bool EnterNum(HWND *ret) {
	...
	lasti = --i;
	std::cout << "Введите номер нужного окна\n";
	std::cin >> i;
	...
	std::cout << "Используется номер " << i << '\n';
	*ret = st[i];
	return 1;
}

bool EnterPath(WCHAR *ret){
		std::cout << "Ведите адрес файла\n";
		std::wstring cin;
		std::wcin >> cin;
		...
		std::cout << "Введён неправильный путь\n";
		return 0;
...
int main() 
{
	...
	std::cout << "\t Началось отслеживание изменений\n";
	...
	while (1) {
				...
				if (!GetFWTime(path, &NewFT)) 
					std::cout << "Ошибка! Пропал доступ к файлу!\n";
				...
				if (NewFT.dwHighDateTime != LastFT.dwHighDateTime || NewFT.dwLowDateTime != LastFT.dwLowDateTime)
				{
					...
					std::cout << "Замечено изменение файла.\n";
					LastFT = NewFT;
				}
				Sleep(350);
			}
